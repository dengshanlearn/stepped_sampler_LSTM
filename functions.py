...
class SteppedSampler(Sampler):
    def __init__(self, sampler, step_size):
    ...
    def __iter__(self):
        for item in self.sampler:
            ...
            # iter(batch)
            idx = 0
            while idx < self.step_size+5:
            #while True:
                step_batch.append(item[idx])
                # print(idx)
                idx += 1
                if len(step_batch) == self.step_size:
                    yield step_batch
                    step_batch = []
                    #if idx == len(self.sampler):
                        #break
                    idx = idx - self.step_size + 2
    ...
    # Somewhat related: see NOTE [ Lack of Default `__len__` in Python Abstract Base Classes ]
        ...
        return len(self.sampler) - self.step_size + 1
